...
<h1><?php echo HEADING_TITLE //图像管理标题！！！?></h1>
<?php
...
?>
            ...
            <td class="main" align="center" valign="top">
              <?php
                echo zen_image_submit('button_display.gif', IMAGE_DISPLAY); //显示！！！
              ?>
            </td>
          </tr>
...
if ($products_filter == '') {
?>
    <h2><?php echo HEADING_TITLE_PRODUCT_SELECT; //请选择要修改图像的商品 ?></h2>
<?php 
} else {
  ...
  $product = $db->Execute("select p.products_id, p.products_model,
    ...
    if ($product->RecordCount() > 0) {
        $pInfo = new objectInfo($product->fields); //objectInfo 获取query的一个attributes
      }
    var_dump($pInfo);
...
?>
      <tr>
        <td colspan="6" class="dataTableContent" align="center"><?php echo TEXT_NO_PRODUCT_IMAGES; //本商品没有图像 ?></td>
      </tr>
<?php 
  }
?>
...
<?php var_dump(DIR_FS_CATALOG . DIR_WS_CLASSES . 'bmz_image_handler.class.php') ?><?php var_dump($action_name) ?>

  <!-- Start Data Edit Pane 选择操作
      很重要的部分
  -->
 ...
 */     
      if ( $action != 'layout_edit' ) {
        ...
        $heading[] = array('text' => '<strong>' . TEXT_INFO_NEW_PHOTO . '</strong>'); //新增图像
      }

      $contents = array('form' => zen_draw_form('image_define', FILENAME_IMAGE_HANDLER, 
        '&products_filter=' . $_GET['products_filter'] . $imgNameStr
        .'&action=save', 'post', 'enctype="multipart/form-data"')); //很重要的link
      // check if this is a master image or if no images exist
      if ($no_images) {
        $contents[] = array('text' => '<strong>'.TEXT_INFO_IMAGE_BASE_NAME.'</strong><br />' ); //图像名称(可选)
        ...
        $dir = @dir(DIR_FS_CATALOG_IMAGES);
            ...
            $contents[] = array('text' => '<br /><strong>'.TEXT_INFO_BASE_DIR.'</strong><br />'.TEXT_INFO_NEW_DIR); //图像目录－给图像选择或设置新目录
            ...
            $contents[] = array('text' => '<strong>'.TEXT_INFO_IMAGE_SUFFIX.'</strong><br />'.TEXT_INFO_USE_AUTO_SUFFIX.'<br />' ); //图像后缀(可选)输入指定后缀，留空将自动生成后缀。
      ...
      $contents[] = array('text' => '<br /><strong>' . TEXT_INFO_DEFAULT_IMAGE . '</strong>&nbsp;&nbsp;<strong class="errorText">(required)</strong><br />' 
          ...
          . zen_draw_input_field('default_image', '', ' size="20" ', false, 'file') . '<br />' . $pInfo->products_image); //缺省图像文件
...
//                . IH_NEW_NAME_COPY_IMAGES . '<br />'
       ...
       $contents[] = array('text' => '<br /><strong>' . TEXT_MEDIUM_FILE_IMAGE . '</strong><br />' . 
          zen_draw_input_field('medium_image', '', ' size="20" ', false, 'file') . '<br />'); //中图像文件 (可选)
      $contents[] = array('text' => '<br /><strong>' . TEXT_LARGE_FILE_IMAGE . '</strong><br />' .
          zen_draw_input_field('large_image', '', ' size="20" ', false, 'file') . '<br />');  //大图像文件 (可选)
      $contents[] = array('align' => 'center', 'text' => '<br />' . zen_image_submit('button_save.gif', IMAGE_SAVE) ); //保存
      break;
    case 'layout_delete':
