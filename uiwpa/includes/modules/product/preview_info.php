...
if (!defined('IS_ADMIN_FLAG')) {
  die('Illegal Access');
    ...
    if (zen_not_null($_POST)) {
      ...
      $products_image_name = $pInfo->products_image; //TODO
    }
...
?>
    <table border="0" width="100%" cellspacing="0" cellpadding="2">
      ...
      <tr>
        <?php //var_dump($no_image_upload);
              //var_dump(DIR_WS_CATALOG_IMAGES .  $products_image_name[1]);
        for($img_i=0;$img_i<=count($products_image_name);$img_i++) { var_dump($products_image_name);
        //var_dump(count($products_image_name));
        //var_dump($products_image_name[0]);
        //var_dump($products_image_name[1]);
        ?>
        <td class="main">
          <?php
//auto replace with defined missing image
            if ($_POST['products_image_manual'] != '') {
              $products_image_name[$img_i]= $_POST['img_dir'] . $_POST['products_image_manual'];
              $pInfo->products_name = $products_image_name[$img_i];

            }
            if ($_POST['image_delete'] == 1 || $no_image_upload and PRODUCTS_IMAGE_NO_IMAGE_STATUS == '1') {
            ...
            if ($products_image_name[$img_i] != '') {
              echo zen_image(DIR_WS_CATALOG_IMAGES . $products_image_name[$img_i], $pInfo->products_name, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'align="right" hspace="5" vspace="5"') . $pInfo->products_description;
            }
          ?>
        </td>
        <?php } ?>
